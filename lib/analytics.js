...
export function createEmptyAnalyticsStore() {
  return cloneValue(createEmptyStore());
}
...
export function shouldTrackBrowserVisitor(userAgent) {
  ...
  if (
    BLOCKED_USER_AGENT_PATTERNS.some((pattern) =>
  ...
  return BROWSER_USER_AGENT_PATTERNS.some((pattern) =>
    pattern.test(normalizedUserAgent),
  );
}
